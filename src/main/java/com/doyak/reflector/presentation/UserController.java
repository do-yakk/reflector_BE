...
public class UserController {
    ...
    @DeleteMapping("users")
    public ResponseEntity<APIResponse<?>> deleteUsers(HttpServletRequest request) {
        HttpSession session = request.getSession(false);
        if (session == null) {
            return ResponseEntity.badRequest().body(APIResponse.errorAPI("Session is null."));
        }
        ...
        try {
            ...
            return ResponseEntity.ok().body(APIResponse.successAPI("User deleted successful.", null));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(APIResponse.errorAPI(e.getMessage()));
        }
    }
